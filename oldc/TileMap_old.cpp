...
namespace Renderer {
    ...
    struct MapSegmentNode {
        ...
        Vector<Renderer::TileTransform> visibleTiles;
    };

    struct TileMap {
        Vector2 mapSize;   // in tiles
        Vector2 tileSize;  // in pixels
        SDL_FRect viewPort; // w/h = constraint; x/y set per-frame from player/camera

        VectorRef<Renderer::Tile> tiles;
        int tileCount = 0;
        Vector<int> mapData;
		...
		Vector<Renderer::TileTransform> visibleTiles; // current frames visible tiles
        // NEW: split tunables
        SegmentSplitConfig splitConfig;
    };

    int g_nextMapId = 0;
    UMap<int, Ref<TileMap>> g_tileMaps;
    ...
    // ---------- tile range ----------
    static void rect_to_tile_range(const SDL_FRect& r, const Vector2& tileSz,
        int mapW, int mapH, int& tx0, int& ty0, int& tx1, int& ty1)
    {
        const float tw = std::max(1.0f, tileSz.x), th = std::max(1.0f, tileSz.y);
    ...
    static int approx_tile_count(const SDL_FRect& r, const Vector2& tileSz, int mapW, int mapH) {
        int tx0, ty0, tx1, ty1;
    ...
    // ---------- split criteria ----------
    static bool should_split(const SDL_FRect& rect, const SDL_FRect& vp, const Vector2& tileSz,
    ...
    static void split_rect(const SDL_FRect& r, const Vector2& tileSz, SDL_FRect out[4]) {
        const float tw = std::max(1.0f, tileSz.x), th = std::max(1.0f, tileSz.y);
    ...
    static void emit_tiles_for_rect(const SDL_FRect& r, const Vector2& tileSz, const Vector2& mapSz,
        const VectorRef<Renderer::Tile>& tiles, const Vector<int>& mapData,
        Vector<Renderer::TileTransform>& out)
    {
    ...
    static int new_node(TileMap& m, const SDL_FRect& r, int parent, uint16_t depth) {
        MapSegmentNode n;
    ...
    static void build_quadtree_recursive(TileMap& m, int nodeIdx)
    {
        ...
        const float tw = std::max(1.0f, m.tileSize.x), th = std::max(1.0f, m.tileSize.y);
    ...
    static inline void updateViewportFromPlayer(TileMap& m, const Vector2& playerPos)
    {
        const float halfW = m.viewPort.w * 0.5f;
        const float halfH = m.viewPort.h * 0.5f;

        const float mapWpx = m.mapSize.x * std::max(1.0f, m.tileSize.x);
        const float mapHpx = m.mapSize.y * std::max(1.0f, m.tileSize.y);
    ...
    void createMapSegments(Ref<TileMap>& mref) {
    ...
    static void gatherFromNodeRecursive(const TileMap& m,
        int nodeIdx,
...
// Collect all segment tiles (world-space) into a flat vector.
    static inline void gatherVisibleTilesWorld(const Ref<TileMap>& mref,
        Vector<Renderer::TileTransform>& out)
    {
        out.clear();
        if (!mref) return;
        const TileMap& m = *mref;
    ...
    static inline void toScreenSpace(const Ref<TileMap>& mref,
        Vector<Renderer::TileTransform>& list)
    {
        if (!mref) return;
    ...
    static inline void centerViewportOnPlayer(Ref<TileMap>& mref, const Vector2& playerPos)
    {
        TileMap& m = *mref;
        const float halfW = m.viewPort.w * 0.5f;
        const float halfH = m.viewPort.h * 0.5f;

        const float tw = std::max(1.0f, m.tileSize.x);
        const float th = std::max(1.0f, m.tileSize.y);
...
bool Renderer::initMap(Vector2 mapSize, Vector2 tileSize, Renderer::TileModel* tiles, int tileCount, int& mapId)
{
	auto tileMap = CreateRef<Renderer::TileMap>();
...
bool Renderer::setMapData(int mapId, SDL_FRect& viewPort, const Vector<int>& mapData)
{
...
void Renderer::setMapTile(int mapId, int x, int y, int tileIndex)
{
}

void Renderer::getMapTile(int mapId, int x, int y, int& tileIndex)
{
}

bool Renderer::loadMap(Vector2 mapSize, Vector2 tileSize, Renderer::TileModel* tiles, int tileCount, const char* mapDataFile, int& mapId)
{
    return true;
}

bool Renderer::saveMap(const char* mapDataFile, int mapId)
{
    return true;
}

bool Renderer::updateMap(float deltaTime, Vector2& viewPosition, int& mapId)
{
    auto it = g_tileMaps.find(mapId);
    if (it == g_tileMaps.end()) return false;

    Ref<TileMap> tileMap = it->second;
    if (!tileMap) return false;
...
void Renderer::renderMap(int mapId)
{
    auto it = g_tileMaps.find(mapId);
...
void Renderer::shutDownMap(int& mapId)
{
}
